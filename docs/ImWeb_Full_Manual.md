...
LFO's own Freq field still goes down to 0.001 Hz. The sweep is centred higher
because the source is usually another LFO covering the whole travel, so it
spends half its time in the bottom half: with a 0.001 floor the modulated LFO
sat below 0.1 Hz — over ten seconds a cycle — for 48% of every cycle, which
reads as the mapping doing nothing. Set a per-mapping `minHz` if you want the
very slow decades back.
