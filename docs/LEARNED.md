...
- 2026-07-30: Assert the RELATIONSHIP a control should have, not "bigger than before", and measure it where the measurement is not saturated — a beam-width test at 60 lines showed only 1.16× across a 6× change because the lattice already filled the frame, which a loose threshold would have called a pass for a knob that did nothing (trigger: wrote `fat > thin*1.5`, it failed on correct code; at 8 lines the true response is linear to within 0.5%).
